  ...
  return {
      ...
      return pollScheduledResult(
        ...
        async (observationSignal) => {
          ...
          const deliveries = requested.deliveries.map((delivery, index) => {
            ...
            if (
              observation.id !== delivery.delivery_id ||
              observation.revision !== delivery.revision
            ) {
              throw new Error(
                "Email delivery changed before its exact result could be observed",
              );
            }
            return {
              ...delivery,
    ...
    async retryEmailDelivery(
      ...
      return pollScheduledResult(
        requested,
        async (observationSignal) => {
          const observation = await apiConfig.httpClient.get<EmailDelivery>(
            path,
            scheduledObservationOptions(options, observationSignal),
          );
          if (
            ...
            throw new Error(
              "Email delivery changed before its exact retry result could be observed",
            );
          }
          return observation;
        },
